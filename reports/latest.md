# Reversal Checker — 2026-09-09 23:52 UTC [END-OF-DAY]
(needs >= 4/6 + mandatory structure & volume; showing only BUY_TRIGGER or >= 5/6 confirmed, 1/21 tickers)

## ORCL  (2026-09-09)  close=161.63  RSI=62.4
  [אמינות היסטורית: אמינות בינונית] בבקטסט (5 שנים אחורה): 11 עסקאות בלתי-תלויות, win rate ל-20 יום=54.5%, תשואה ממוצעת ל-20 יום=5.13%.
  [PASS] 1. Price structure (HL+HH)
         HigherLow=True, brokeSwingHigh=True (last swing high=153.99)
  [PASS] 2. Moving averages
         close>152.48(MA10) & >149.69(MA20)=True, MA10 rising=True, MA10>MA50=True
  [PASS] 3. MACD
         MACD=3.97 vs Signal=2.35 (above=True), hist=1.62 rising=True
  [PASS] 4. RSI
         RSI=62.39 rising=False, bullishDivergence=False
  [----] 5. Volume / RVOL
         day up=False, RVOL=1.14, up>downVol=False
  [PASS] 6. Volume Profile POC
         close=161.63 vs POC=142.95 (above=True)
  >> 5/6 confirmed | mandatory(structure+volume)=False
  >> NO ENTRY - wait for confirmation
